...
연합뉴스 7개 RSS
  ...
  → 선택 기사만 원문 수집
  → Luna: 기사별 관측 0~3개, 직접 근거 문단 ID, 기억 여부·선정 이유 추출
...
질문 설계에서는 핵심 대상·거래 방식·약어의 뜻을 먼저 묻고, 이유·작동 과정 질문도 유지한다. `currentEvidence`에는 원문 관측의 직접 근거 외에 앞뒤 한 문단을 보충하되, 원문 해시 일치·중복 제거·추가 문단 실행 전체 2,500자 상한을 적용한다. 주변 문단은 질문 발견용이며 connection의 새 사건 근거나 장기 기억으로 승격하지 않는다. 전체 Terra 입력 15,000 tokens·비용 가드는 그대로 적용한다. 이 제한 밖 문단의 개념 누락은 아직 남을 수 있다.
...
질문 검색은 별도로 질문당 상위 3기사, 실행 전체 8기사·8,000자까지 전달한다. 추가 검색 질문은 최대 8개로 제한하고 필요한 질문 자체를 삭제하지 않는다. 원리 예산은 실제 선택한 것 기준으로 4청크·4,000자다. 설계에서 버린 원리가 질문 검색 예산을 차지하지 않는다. 검색 문자열은 중복 제거해 일괄 임베딩한다. 같은 근거는 편집 입력의 catalog에 한 번만 넣고 각 행에 `allowedAnswers`를 표시한다. 본문은 flowEvidence 범위에서 작성한다. 편집 상세도는 medium이며 전체 날짜의 질문·답변을 위한 출력 상한은 12,000 tokens다. API 호출과 비용 사전 계산이 같은 상수를 사용한다. 입력의 질문 ID는 Q 태그에 한 번만 표시하며 입력 15,000 tokens·일일 비용 상한은 유지한다.
...
기사 ID는 LLM 호출마다 `A001` 같은 짧은 별칭으로 바꾸고, 시각도 `HH:mm`만 보낸다. 제목 사전선별은 같은 6시간 안에서도 시간대별 기사를 교차 배열해 목록 위치 편향을 줄인다. 원문은 선택된 기사의 관측 추출에만 한 번 보낸다. 기사 ID·본문 해시·URL·발행시각·모델·추출 프롬프트 버전이 일치한 관측과 임베딩만 재사용한다. RSS 메타가 바뀌면 원문 상태를 무효화해 재수집한다. 추출 기준 변경 시 `EXTRACTION_PROMPT_VERSION`을 올린다.
