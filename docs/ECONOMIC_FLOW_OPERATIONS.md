...
질문 설계에서는 핵심 대상·거래 방식·약어의 뜻을 먼저 묻고, 이유·작동 과정 질문도 유지한다. `currentEvidence`에는 원문 관측의 직접 근거 외에 앞뒤 한 문단을 보충하되, 원문 해시 일치·중복 제거·추가 문단 실행 전체 3,000자 상한을 적용한다. 주변 문단은 질문 발견용이며 connection의 새 사건 근거나 장기 기억으로 승격하지 않는다. 전체 Terra 입력 15,000 tokens·비용 가드는 그대로 적용한다. 이 제한 밖 문단의 개념 누락은 아직 남을 수 있다.
...
편집 응답의 flows 배열 길이는 해당 실행에서 Terra가 설계한 길이를 JSON Schema로 강제한다. 날짜별 흐름 개수를 미리 정하지 않는다. 본문·답변의 수치와 질문별 근거 ID 검증은 유지하며 편집기가 새 수치 예시·단위 환산을 추가하지 않도록 지시한다. 답변에 쓴 수치가 질문에 허용된 다른 근거에 정확히 있으면 코드가 해당 근거 ID를 보완하고, 허용 근거 어디에도 없는 수치는 계속 실패시킨다.
