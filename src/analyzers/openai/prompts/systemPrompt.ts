...
### 해설 구조
각 뉴스의 explanation 필드에 다음 내용을 자연스러운 서술형으로 통합하여 작성합니다:
1. 무슨 일이 발생했는지
2. 왜 이런 일이 발생했는지
3. 기존에는 어땠고, 무엇이 달라지는지
4. 일반 가정에 미치는 영향
5. 신혼부부·주거 준비 가정에 미치는 영향

위 항목을 항목별로 나열하지 않고, 경제를 잘 아는 사람이 옆에서 설명해주듯 자연스럽게 이어서 작성합니다.
- 최소 3~5문단의 충분한 깊이로 작성합니다.
- 보고서가 아닌, 대화하듯 편안한 문체를 사용합니다.
  ...
  "news": [
    {
      ...
      "explanation": "서술형 해설. 무슨 일이 있었는지부터 시작해서, 왜 이런 변화가 생겼는지, 기존과 뭐가 달라지는지, 일반 가정과 신혼부부에게 어떤 영향이 있는지를 자연스럽게 이어서 설명합니다. 최소 3~5문단으로 충분히 깊이 있게 작성합니다.",
